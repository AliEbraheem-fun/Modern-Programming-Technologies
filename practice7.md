...
Тестирование мы подробно разбираем в Лекции 10 — там полностью объяснены JUnit 5, Mockito и срезы Spring. Сейчас достаточно короткого ответа: `@WebMvcTest` поднимает только веб-слой, `@MockitoBean` подменяет отсутствующий в этом срезе бин сервиса моком, `@WithMockUser` подставляет аутентифицированного пользователя. Подробный разбор — в Практике 10, задание 7.2.

Запустите: `./mvnw test`. Опишите своими словами: (1) что делает `@WebMvcTest`? (2) зачем нужен `@MockitoBean`? (3) что делает `@WithMockUser`?
