...
Collectors и группировку по ключу разберём подробно в Лекции 6, часть 6 — здесь тренируемся на более простых операциях.
...
5. Получите список названий товаров дороже 1000 руб., без дубликатов, в верхнем регистре
...
## Часть 8: Работа с датами и временем (Java Time API)

Во всех заданиях этой части считайте от фиксированных дат, а не от `LocalDate.now()`. Иначе вывод будет меняться каждый день и проверить работу станет невозможно.

### Задание 8.1: Возраст и стаж — Period против ChronoUnit

Создайте файл `AgeCalculator.java`.

**Требования:**

1. `record Employee(String name, LocalDate birthDate, LocalDate hireDate)`
2. Метод `static String formatPeriod(LocalDate from, LocalDate to)` — берёт `Period.between(from, to)` и возвращает строку вида `"20 г. 6 мес. 7 дн."`
3. Метод `static long totalDays(LocalDate from, LocalDate to)` — общее число дней через `ChronoUnit.DAYS`
4. Метод `static long fullYears(LocalDate from, LocalDate to)` — число полных лет через `ChronoUnit.YEARS`
5. Константа `static final LocalDate REPORT_DATE = LocalDate.of(2026, 3, 8)` — дата, на которую считается отчёт
6. В `main` создайте список из трёх сотрудников и выведите таблицу
7. Найдите сотрудника с наибольшим стажем (сравнивайте по `totalDays`)

Данные сотрудников:

| Имя | Дата рождения | Дата приёма |
|-----|---------------|-------------|
| Алиса | 2005-09-01 | 2024-02-12 |
| Борис | 1998-12-31 | 2019-07-01 |
| Вера | 2003-03-08 | 2022-11-30 |

**Ожидаемый вывод:**
```
=== Отчёт на 08.03.2026 ===
Алиса  | возраст: 20 г. 6 мес. 7 дн. | стаж: 2 г. 0 мес. 24 дн. | дней стажа: 755  | полных лет: 2
Борис  | возраст: 27 г. 2 мес. 8 дн. | стаж: 6 г. 8 мес. 7 дн.  | дней стажа: 2442 | полных лет: 6
Вера   | возраст: 23 г. 0 мес. 0 дн. | стаж: 3 г. 3 мес. 8 дн.  | дней стажа: 1194 | полных лет: 3

Больше всех проработал: Борис (2442 дн.)
```

**Вопрос:** Возьмите стаж Бориса и посчитайте «общее число дней» вручную: `getYears() * 365 + getMonths() * 30 + getDays()`. Сравните с `ChronoUnit.DAYS.between()`. Откуда взялось расхождение и почему `Period` в принципе нельзя «схлопнуть» в дни без обеих дат?

### Задание 8.2: Расписание и перенос дедлайна

Создайте файл `ScheduleBuilder.java`.

**Требования:**

1. `record Lesson(String title, DayOfWeek day, LocalTime start, Duration length)`
2. Метод `static LocalTime end(Lesson lesson)` — время окончания занятия (`start.plus(length)`)
3. Метод `static LocalDate nextDate(Lesson lesson, LocalDate from)` — ближайшая дата занятия строго после `from`, через `TemporalAdjusters.next(lesson.day())`
4. Метод `static Duration weekTotal(List<Lesson> lessons)` — суммарная длительность всех занятий за неделю (начните с `Duration.ZERO` и складывайте через `plus`)
5. Метод `static TemporalAdjuster nextBusinessDay(Set<LocalDate> holidays)` — возвращает собственный корректировщик, который сдвигает дату на следующий день и продолжает сдвигать, пока не попадёт на день, который не суббота, не воскресенье и не входит в множество праздников
6. В `main`: расписание из трёх занятий (данные ниже), отсчёт от `LocalDate.of(2026, 3, 8)`, праздники — `Set.of(LocalDate.of(2026, 3, 9))`, дедлайн — `LocalDate.of(2026, 3, 7)`
7. Названия дней недели выводите через `day.getDisplayName(TextStyle.FULL_STANDALONE, Locale.forLanguageTag("ru"))`, даты — через `DateTimeFormatter.ofPattern("dd.MM.yyyy")`

Расписание:

| Занятие | День | Начало | Длительность |
|---------|------|--------|--------------|
| Лекция по Java | MONDAY | 10:30 | 95 минут |
| Практика по Java | WEDNESDAY | 13:00 | 95 минут |
| Консультация | FRIDAY | 16:20 | 45 минут |

**Ожидаемый вывод:**
```
=== Расписание, отсчёт от 08.03.2026 (воскресенье) ===
Лекция по Java   | понедельник | 10:30-12:05 | ближайшая дата: 09.03.2026
Практика по Java | среда       | 13:00-14:35 | ближайшая дата: 11.03.2026
Консультация     | пятница     | 16:20-17:05 | ближайшая дата: 13.03.2026

Всего занятий в неделю: 3 ч 55 мин

Дедлайн 07.03.2026 (суббота) переносится на 10.03.2026 (вторник)
```

**Вопрос:** Дедлайн сдвинулся на вторник, а не на понедельник. Почему? И что вернёт `TemporalAdjusters.next(DayOfWeek.SUNDAY)`, если применить его к воскресенью, — ту же дату или следующее воскресенье?

### Задание 8.3: Форматирование, разбор и часовые пояса

Создайте файл `TimeZoneConverter.java`.

**Требования:**

1. Поле `private static final DateTimeFormatter RU_FORMAT = DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm")` — один экземпляр на весь класс
2. Метод `static ZonedDateTime parseMoscow(String text)` — разбирает строку в `LocalDateTime` и привязывает её к `ZoneId.of("Europe/Moscow")` через `atZone`
3. Метод `static void printInZones(ZonedDateTime moment)` — печатает момент в поясах `Europe/Moscow`, `Asia/Tokyo`, `America/New_York` (через `withZoneSameInstant`) и его `Instant`
4. Метод `static String inWords(LocalDate date)` — возвращает «8 марта 2026 (воскресенье)» через `DateTimeFormatter.ofPattern("d MMMM yyyy (EEEE)", Locale.forLanguageTag("ru"))`
5. В `main` обработайте массив строк `{"08.03.2026 14:00", "01.09.2026 09:00", "2026-03-08 14:00"}`. Последняя строка написана в ISO-формате и под шаблон не подходит — поймайте `DateTimeParseException` и выведите понятное сообщение, не роняя программу

**Ожидаемый вывод:**
```
=== 08.03.2026 14:00 ===
Europe/Moscow    : 08.03.2026 14:00
Asia/Tokyo       : 08.03.2026 20:00
America/New_York : 08.03.2026 07:00
Instant (UTC)    : 2026-03-08T11:00:00Z
Словами          : 8 марта 2026 (воскресенье)

=== 01.09.2026 09:00 ===
Europe/Moscow    : 01.09.2026 09:00
Asia/Tokyo       : 01.09.2026 15:00
America/New_York : 01.09.2026 02:00
Instant (UTC)    : 2026-09-01T06:00:00Z
Словами          : 1 сентября 2026 (вторник)

=== 2026-03-08 14:00 ===
Не удалось разобрать строку по шаблону dd.MM.yyyy HH:mm
```

**Вопрос:** (1) Добавьте в массив строку `"31.02.2026 10:00"` и запустите. Программа упала или выдала дату? Объясните результат. (2) Почему `DateTimeFormatter` безопасно держать в `static final`-поле, а `SimpleDateFormat` — нет? (3) Чем `yyyy` отличается от `YYYY` в шаблоне и в какие дни года разница станет заметна?

---

## Часть 9: Интеграционное задание

### Задание 9.1: Система управления библиотекой
...
## Часть 10: Эксперименты в jshell

### Задание 10.1: Sealed-классы
...
### Задание 10.3: Сравнение EnumSet и HashSet
...
**11.** Что выведет этот код? Объясните, почему именно так.
```java
...
**12.** У вас есть две даты: `2005-09-01` и `2026-03-08`. Какой инструмент вы возьмёте, чтобы (а) показать пользователю «20 лет 6 месяцев 7 дней», (б) посчитать, сколько всего дней прошло, (в) измерить длительность пары в минутах? Объясните разницу между `Period`, `Duration` и `ChronoUnit`.

**13.** Приложение записывает время события в базу данных. Почему для этого берут `Instant`, а не `LocalDateTime`? Что сломается, если сервер переедет в другой часовой пояс или страна отменит переход на летнее время?
...
1. Файлы `.java` со всеми выполненными заданиями (Части 1–9)
2. Ответы на контрольные вопросы (Часть 11)
3. Заметки о результатах экспериментов в jshell (Часть 10)
