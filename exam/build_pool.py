...
MANUAL_DROP_QUESTIONS = {
    ...
    # Batch of 17 from teacher review:
    'Чем sealed-интерфейс отличается от sealed-класса?',
    'Какой класс корректно наследует обобщённый Box<T> с фиксированным типом String?',
    'Что такое полиморфизм в контексте абстрактных классов?',
    'Что особенного в текстовых блоках (text blocks)?',
    'Чем UnaryOperator<T> отличается от Function<T, R>?',
    'Чем EnumSet лучше обычного HashSet для хранения enum-значений?',
    'Может ли анонимный класс иметь собственный конструктор?',
    'Что произойдёт в компактном конструкторе record при выбросе исключения?',
    'Что означает правило PECS (Producer Extends, Consumer Super)?',
    'Что такое Function<T, R>?',
    'Какой интерфейс является корнем иерархии коллекций в Java (JCF)?',
    'Что произойдёт при конфликте default-методов из двух интерфейсов?',
    'Может ли аннотация @FunctionalInterface быть применена к интерфейсу с несколькими default-методами?',
    'Какой результат выражения 5  (сдвиг влево)?',  # double space — `<<` stripped by HTML cleanup
