...
Класс `SmartDevice` выше одновременно наследует абстрактный `Device` и реализует два интерфейса — так на одной диаграмме видно все связи, которые в коде разбросаны по разным строкам. Схема ниже сводит их вместе: откуда берётся метод `turnOn()`, что обязывают реализовать интерфейсы `Connectable` и `Updatable`, и где среди полей и методов прячется вложенный класс `FirmwareVersion`.
...
В классе `SmartDevice` выше на самом деле сразу несколько разных вещей — не только поля и методы, но и константа, статический счётчик и блок инициализации. Разберём, чем они отличаются друг от друга.
...
Соберём всё сказанное в одном примере: интерфейс `Shape` явно перечисляет в `permits`, кто имеет право его реализовать, а каждый из трёх наследников выбирает свой способ закрыть или, наоборот, продолжить иерархию — через `record` (неявно `final`), через явный `final` или через `non-sealed`.
...
Из этого определения следует несколько практических ограничений, которые легко нарушить по привычке от обычных классов:
...
В коде выше легко потерять из виду, откуда у `Robot` взялся `connectToWiFi()` — он приходит не напрямую, а через `SmartDevice`, который сам собран из двух независимых интерфейсов. Диаграмма показывает эту цепочку целиком: `Movable` и `Powered` расширяются в `SmartDevice`, а класс `Robot` реализует уже готовую комбинацию.
...
Java унаследовала от C синтаксис со скобками после имени переменной, но не стоит им пользоваться — он путает читателя, заставляя искать тип то до, то после имени. Ниже — три равнозначных способа создать массив, от устаревшего к предпочитаемым.
...
Каждый из этих методов возвращает НОВУЮ строку — сам объект `String` не изменяется, ведь строки в Java неизменяемы:
...
Лучший аргумент в пользу record — просто положить два варианта рядом. Слева — всё, что пришлось бы написать вручную для неизменяемого класса с двумя полями: конструктор, геттеры, `equals()`, `hashCode()`, `toString()`. Справа — то же самое одной строкой.
...
Компилятор добавляет эти методы автоматически к любому `enum` — писать их вручную не нужно:
...
Аннотация `@Retention` определяет, как долго живёт информация об аннотации — до какого этапа сборки и выполнения программы она доживает:
...
Оба типа объявляются прямо внутри метода, но выбор между ними зависит от того, нужно ли классу имя и понадобится ли он больше одного раза:
...
`java.util.function` уже содержит готовые интерфейсы почти на любой случай, так что свой функциональный интерфейс приходится писать редко:
...
Java различает, НА ЧТО именно ссылается запись `Класс::метод` — на статический метод, метод конкретного объекта, метод произвольного объекта данного типа или на конструктор:
...
До Java 9 все `.jar`-файлы просто сваливались в один общий classpath, и с этим было связано сразу несколько застарелых болячек платформы:
...
Теперь применим директивы из таблицы к реальному модулю: он требует несколько чужих модулей и экспортирует наружу собственные пакеты, чтобы классы из них можно было использовать за пределами модуля.
...
У любого объекта `LocalDate`/`LocalDateTime` можно спросить отдельные компоненты — год, месяц, день недели. Обратите внимание: `getMonth()` и `getDayOfWeek()` возвращают не числа, а перечисления (`Month`, `DayOfWeek`), с которыми удобнее сравнивать и переключаться через `switch`, чем с «магическими» цифрами 1–12 или 1–7.
