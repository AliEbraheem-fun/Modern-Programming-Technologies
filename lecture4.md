...
| Локальный класс | (внутри метода) | К локальным переменным (effectively final) |
| Анонимный класс | `new Interface() {}` | К effectively final переменным |
...
**Ограничение:** Нестатический внутренний класс **не может содержать статических членов** (статических полей, методов и вложенных классов), за исключением `static final` констант времени компиляции:
```java
class Outer {
    class Inner {
        static final int CONSTANT = 42;  // OK — константа времени компиляции
        // static int counter = 0;       // Ошибка! Нельзя в нестатическом внутреннем классе
    }
}
```
...
### 1.4 Локальный класс (Local Class)

Локальный класс объявляется **внутри метода**. Он виден только в пределах этого метода и имеет доступ к локальным переменным метода, но **только если они effectively final** (не изменяются после инициализации):

```java
public void processData(String prefix) {
    final int multiplier = 10; // effectively final — можно использовать в локальном классе

    class Formatter {
        String format(int value) {
            // Доступ к параметру prefix и локальной переменной multiplier
            return prefix + ": " + (value * multiplier);
        }
    }

    Formatter f = new Formatter();
    System.out.println(f.format(5)); // "Результат: 50"
}
```

**Переменная effectively final** — это переменная, которая не изменяется после присваивания, даже если ключевое слово `final` не указано явно.

### 1.5 Анонимный класс (Anonymous Class)

Анонимный класс — это **безымянный** класс, который объявляется и создаётся в одном выражении. Используется для одноразовой реализации интерфейса или расширения класса:

```java
// Анонимная реализация интерфейса Runnable
...
animal.speak(); // Кот мяукает.
```

**Особенности анонимных классов:**
- **Не имеют имени** — нельзя повторно использовать в другом месте
- **Не могут иметь явно объявленный конструктор** (поскольку у них нет имени)
- Могут обращаться только к **effectively final** переменным окружающего контекста
- Часто заменяются **лямбда-выражениями** (для функциональных интерфейсов с одним методом)
