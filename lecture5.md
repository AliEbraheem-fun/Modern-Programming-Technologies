...
`List` запоминает порядок добавления и допускает повторы — если добавить один и тот же элемент дважды, он появится в списке дважды.
...
Тот же список, но обёрнутый в `HashSet`: если бы среди элементов был повтор, `Set` тихо отбросил бы лишнюю копию — это его главное отличие от `List`.
...
`Map` хранит не сами элементы, а пары «ключ — значение»; повторная запись по уже существующему ключу перезапишет старое значение, а не добавит вторую запись.
...
Обычная `Queue` отдаёт элементы в том порядке, в котором они пришли (FIFO). `PriorityQueue` — исключение: она игнорирует порядок добавления и всегда отдаёт наименьший элемент, поэтому в примере ниже `poll()` возвращает `1 3 5`, а не `5 1 3`.
