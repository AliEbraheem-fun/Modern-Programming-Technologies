...
### 3.5 Проблемы многопоточности и синхронизация
...
### 3.6 Ключевое слово synchronized
...
`synchronized` гарантирует, что только один поток в момент времени выполняет синхронизированный блок/метод:
...
### 3.8 Атомарные классы (java.util.concurrent.atomic)

Напоследок рассмотрим ещё один подход к безопасной работе с данными из нескольких потоков. Иногда `synchronized` — это слишком тяжёлый инструмент для простых операций вроде инкремента счётчика. В таких случаях удобнее использовать атомарные классы:
...
// Результат всегда будет корректным без synchronized
```
...
| Поток | `Thread`, `Runnable`, `start()`, `join()`, `sleep()` |
| Синхронизация | `synchronized` (монитор), `wait()`, `notify()`, `notifyAll()` (из `Object`), `AtomicInteger` |
