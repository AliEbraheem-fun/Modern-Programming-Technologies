...
Вы уже видели комментарии в коде выше — строки, начинающиеся с `//`. Комментарий не выполняется, это пояснение для человека, а не для компьютера. В Java есть три вида комментариев:
...
/**
 * Документирующий комментарий (Javadoc)
 * — описывает класс, метод или поле
 */
```

Хорошее правило: комментарий должен объяснять **почему** сделано именно так, а не **что** делает код — само название переменной или метода уже должно быть понятным без перевода. Комментарий вида `// увеличиваем i на 1` рядом со строкой `i++` бесполезен, а `// компенсируем смещение, т. к. нумерация мест начинается с 1` — уже даёт полезный контекст.

Подробно про Javadoc и документирующие комментарии поговорим в Лекции 10.
...
Библиотека скомпилирована, осталось указать JVM, где её искать — параметр `-Djava.library.path` задаёт каталог для поиска нативных `.dll`/`.so`/`.dylib` файлов.
...
В примере выше участвовало сразу несколько незнакомых конструкций — соберём их в одну шпаргалку:
...
Между примитивными числовыми типами Java допускает преобразования (conversions) — но по-разному, в зависимости от направления.

**Расширяющее преобразование (widening)** происходит неявно — компилятор сам вставляет его, потому что данные точно поместятся в больший тип:
...
```java
int i = 100;
long l = i;     // int спокойно помещается в long
double d = l;   // а long — в double
```

Аналогия: переливаете стакан воды в ведро — оно точно вместит, спрашивать разрешения не нужно.

**Сужающее преобразование (narrowing)** — обратная операция. Она может привести к потере данных, поэтому требует явного приведения типа — оператора `(тип)`:

```java
double x = 3.9;
int y = (int) x;       // 3 — дробная часть отбрасывается, а не округляется
long big = 300L;
byte b = (byte) big;   // 44 — значение "обрезается" до диапазона byte
```

Аналогия: пытаетесь перелить ведро в стакан — компилятор заставляет вас явно, через `(int)`, согласиться, что часть воды прольётся.

**`char` в арифметике.** Как только `char` попадает в арифметическое выражение, он автоматически продвигается до `int`:

```java
char c = 'A';
int code = c + 1;            // 66 — 'A' превращается в свой код 65
char next = (char) (c + 1);  // 'B' — обратно нужно приводить явно
```

**Продвижение операндов.** Если хотя бы один из операндов — `byte`, `short` или `char`, оба продвигаются минимум до `int`, поэтому результат сложения двух `byte` — уже не `byte`:

```java
byte n1 = 10, n2 = 20;
// byte sum = n1 + n2;   // ошибка компиляции: результат имеет тип int
int sum = n1 + n2;        // так — верно
```

**Скрытое приведение в составном присваивании.** Операторы `+=`, `-=` и подобные содержат неявное сужающее приведение:

```java
byte value = 10;
value += 300;               // на деле выполняется: value = (byte) (value + 300)
System.out.println(value);  // 54, а не 310 — лишние биты обрезаны
```

Именно поэтому `value += 300` компилируется, а `value = value + 300` — нет: во втором случае компилятор `(byte)` за вас не добавляет.
...
Составные операторы присваивания — это сокращённая запись «взять переменную, что-то с ней сделать и сохранить обратно в неё же»:
