# Практическое занятие 4: Вложенные классы, Обобщения, Исключения и Отладка
...
### Задание 6.1: Отладка AverageCalculator

В лекции разбирался метод, который считает среднее по списку строк, пропуская те, что не разбираются в число. Программа не падает, но выдаёт неверный ответ — классический случай для отладчика, а не для компилятора.

```java
package ru.fin.debug;
...
**Шаг 1. Воспроизведите ошибку.** Запустите класс обычным способом (без отладки). Убедитесь, что программа печатает `Среднее: 15.0`, хотя среднее от 10, 20 и 30 равно `20.0`.

**Шаг 2. Поставьте точку останова на результате.** Щёлкните по левому полю редактора напротив строки `return sum / count;` и запустите класс в режиме отладки (Shift+F9 в IntelliJ IDEA, F5 в VS Code). Когда программа остановится, откройте окно **Variables** и запишите значения `sum` и `count`.

**Шаг 3. Пройдите цикл по шагам.** Перезапустите отладку с точкой останова на строке `for (CharSequence row : rows) {`. Двигаясь **Step Over** (F8 / F10), пройдите тело цикла итерацию за итерацией и на каждом шаге фиксируйте в окне Variables значения `row`, `sum`, `count`. На строке `Double.parseDouble(...)` один раз примените **Step Into** (F7 / F11), чтобы увидеть, куда уводит вызов, а затем вернитесь в свой код через **Step Out** (Shift+F8 / Shift+F11).

**Шаг 4. Поставьте условную точку останова.** На строке `count++;` откройте свойства точки останова и в поле **Condition** укажите `row.toString().equals("abc")`. Перезапустите отладку — теперь она остановится сразу на нужной итерации, не заставляя проходить первые две вручную.

**Шаг 5. Проверьте гипотезу через Evaluate Expression.** Не выходя из отладки (можно снова остановиться на `return sum / count;`), откройте **Evaluate Expression** (Alt+F8 в IntelliJ IDEA, панель Debug Console в VS Code) и вычислите выражение `sum / (count - 1)`. Сравните результат с ожидаемым ответом `20.0` — ничего при этом не перекомпилируя.

**Ответьте письменно:**
1. После какой строки в теле цикла значения `sum` и `count` ещё соответствуют вашим ожиданиям, а после какой строки — уже нет? Укажите конкретную строку кода.
2. Почему `sum` в момент выхода из цикла равна `60.0` (то есть верна), а `count` — нет?
3. На какой итерации сработала условная точка останова с условием `row.toString().equals("abc")` и какие значения `sum`/`count` были на тот момент?
4. Найдите и исправьте ошибку в методе `average`. Приложите исправленный код и результат повторного запуска.
...
11. Чем условная точка останова отличается от обычной и когда она удобнее?
12. Зачем в отладчике нужна отдельная точка останова на исключении (Exception Breakpoint), если стек-трейс и так печатается в консоли?
13. Что показывает окно Variables при остановке на строке, и чем это отличается от вывода через `println()`?
...
2. Найденную и исправленную ошибку в `AverageCalculator` вместе с письменными ответами по отладке (Задание 6.1)
3. Ответы на контрольные вопросы
4. Краткое описание того, какие задачи вызвали затруднения
