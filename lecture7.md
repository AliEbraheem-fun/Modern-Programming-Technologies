...
Здесь зависимость не обязательна на момент создания объекта — Spring вызывает сеттер после конструктора, поэтому поле нельзя сделать `final`, зато его можно поменять и позже, например в тестах.
...
`scope` определяет, сколько экземпляров бина создаёт контейнер и как долго они живут — по умолчанию Spring создаёт бин ровно один раз на всё приложение:
...
Соберём все мэппинги, которые мы разбирали по отдельности, в один рабочий контроллер — с созданием, чтением, обновлением и удалением студентов:
...
Все `th:`-атрибуты работают по одному принципу: они читают значение из модели, которую передал контроллер, и подставляют его в обычный HTML-тег. Ниже — набор самых часто используемых атрибутов на одной странице: вывод текста, условие, цикл, ссылка и форма.
...
Эту самую цепочку фильтров вы настраиваете сами, объявив бин `SecurityFilterChain`: какие пути открыты всем, какие требуют определённой роли, и что делать с формой входа.
...
Лекция получилась длинной — соберём весь путь от простого DI до полноценного production-ready сервиса в одну таблицу:
