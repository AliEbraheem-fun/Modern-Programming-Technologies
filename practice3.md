...
### Задание 3.3: От наследования к композиции

Возьмите класс `StackByInheritance` из лекции 3 (раздел 3.2) — он расширяет `ArrayList`. Перепишите его в `StackByComposition`, спрятав список за приватным полем `items`, как показано в лекции. У нового класса должны остаться только методы `push`, `pop`, `peek` и `isEmpty` — никаких других публичных методов списка быть не должно.

Напишите `main`, который демонстрирует разницу на практике:
1. Создайте `StackByInheritance<Integer>` и `StackByComposition<Integer>`, наполните оба тремя элементами через `push`.
2. Вызовите у `StackByInheritance` унаследованный метод `add(0, 999)` — элемент окажется в середине стека вопреки его контракту — и покажите, что после этого `pop()` возвращает не то значение, которое было добавлено последним.
3. Покажите, что для `StackByComposition` строка `stack.add(0, 999)` вообще не компилируется — у класса просто нет такого метода.

**Ответьте письменно:** (1) Какое отношение в реальности связывает стек и список — is-a или has-a — и почему `extends ArrayList` здесь вводит в заблуждение? (2) Какие публичные методы «утекли» наружу из-за наследования и почему это ломает абстракцию стека? (3) Если внутри `StackByComposition` заменить `ArrayList` на `LinkedList`, придётся ли менять сигнатуры `push`, `pop` и `peek`? Что из этого следует про гибкость композиции?
