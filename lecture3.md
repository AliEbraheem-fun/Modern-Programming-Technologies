...
### 3.2 Отношения между классами: наследование, композиция, агрегация, зависимость

Мы разобрали наследование как способ сказать «этот класс — это разновидность того класса» (`Dog extends Animal`). Такое отношение называют **is-a** («является»). Но далеко не всё в программе связано через «является». Гораздо чаще один класс просто **пользуется** другим или **содержит** его как часть — и для этого есть три других вида связи: композиция, агрегация и зависимость. Их объединяет общее название **has-a** («имеет»), кроме зависимости, которая устроена ещё слабее.

**Аналогия:** Представьте квартиру. Комнаты — неотъемлемая часть квартиры: если дом снесут, комнаты исчезнут вместе с ним, отдельно от квартиры они не существуют — это **композиция**. Мебель в квартире — совсем другое дело: диван можно вынести и поставить в другую квартиру, он существует независимо от того, в какой квартире стоит сейчас — это **агрегация**. А курьер, который на пять минут заходит, чтобы отдать заказ, вообще не становится частью квартиры и не остаётся в ней жить — это **зависимость**: временная связь «использует», а не «содержит».

Разберём каждое отношение подробнее:

- **Композиция (composition)** — целое владеет частью: создаёт её и уничтожает вместе с собой. Часть не имеет смысла без целого.
- **Агрегация (aggregation)** — целое содержит ссылку на часть, но не управляет её жизненным циклом: часть создана снаружи и продолжает существовать, даже если целое исчезнет.
- **Зависимость (dependency)** — самая слабая связь: один класс использует другой мимоходом (обычно как параметр метода), не храня на него постоянную ссылку.

```java
// Композиция: Engine создаётся внутри Car и не существует отдельно от неё
class Car {
    private final Engine engine = new Engine(); // Car владеет Engine
}

// Агрегация: Passenger существует независимо от Car
class Car2 {
    private final List<Passenger> passengers;

    public Car2(List<Passenger> passengers) {
        this.passengers = passengers; // передан снаружи, Car2 его не создавала и не уничтожает
    }
}

// Зависимость: Car использует Mechanic мимоходом, ссылку не сохраняет
class Car3 {
    public void repair(Mechanic mechanic) { // временная связь — параметр метода
        mechanic.fix(this);
    }
}
```

#### Одна задача — два решения: наследование против композиции

Рассмотрим классическую ошибку проектирования на примере стека. Кажется естественным сказать: «стек — это список, в который можно добавлять и убирать элементы только с одного конца», и реализовать его через наследование:

```java
// Вариант 1: наследование — StackByInheritance "является" ArrayList
class StackByInheritance<T> extends ArrayList<T> {
...
На первый взгляд работает. Но `extends ArrayList<T>` тянет за собой **все** публичные методы списка — `add(int, T)`, `remove(int)`, `set(int, T)` и так далее. Ничто не мешает вызвать `stack.add(0, item)` и вставить элемент в середину «стека», хотя по определению стек обязан работать только с одним концом. Наследование раскрыло наружу внутреннюю реализацию и сломало абстракцию.

Теперь то же самое через композицию — список становится не родителем, а полем:

```java
// Вариант 2: композиция — StackByComposition "содержит" список внутри
class StackByComposition<T> {
    private final List<T> items = new ArrayList<>(); // поле, а не родитель
    ...
    public T pop() {
        if (items.isEmpty()) throw new IllegalStateException("Стек пуст");
...
Теперь наружу видны только `push`, `pop`, `peek` и `isEmpty` — ровно тот контракт, который нужен стеку. Более того, `items` можно в любой момент заменить на `LinkedList<>()` или другую реализацию `List` — снаружи это будет незаметно, потому что реализация спрятана за полем, а не выставлена наследованием.

#### Правило: предпочитайте композицию наследованию

Это не значит, что наследование не нужно — там, где действительно есть отношение is-a и вы контролируете оба класса (как `Dog extends Animal`), оно уместно. Но когда вы хотите просто переиспользовать чужой код, композиция почти всегда безопаснее. Причины:

1. **Наследование фиксируется при компиляции.** Родительский класс, указанный после `extends`, нельзя сменить во время выполнения программы. Композиция же хранит ссылку в поле — её можно передать через конструктор или сеттер и при необходимости подменить другой реализацией.
2. **Хрупкость базового класса (fragile base class problem).** Если автор родительского класса изменит его внутреннюю реализацию — например, метод `addAll()` начнёт вызывать `add()` изнутри, — поведение всех подклассов может незаметно измениться, хотя видимый контракт родителя не поменялся. Композиция от этого защищена: пока внешний интерфейс зависимого объекта не меняется, его внутренние переделки никак не задевают класс, который его использует.
3. **Инкапсуляция.** Наследование выставляет наружу все `public` и `protected` члены родителя. Композиция позволяет выбрать, какие методы делегировать, а какие оставить скрытыми.

| Критерий | Наследование (`extends`) | Композиция (поле) |
|----------|---------------------------|---------------------|
| Тип связи | is-a («стек — это список») | has-a («стек содержит список») |
| Жёсткость связи | Жёсткая — родитель задан раз и навсегда в коде класса | Гибкая — объект-поле можно передать снаружи и заменить |
| Подмена реализации во время выполнения | Невозможна | Возможна — через конструктор или сеттер можно подставить другую реализацию интерфейса |
| Хрупкость базового класса | Высокая — изменение родителя способно сломать все подклассы | Низкая — внутренние изменения зависимого класса не затрагивают внешний, пока сохраняется его интерфейс |
| Что видно снаружи | Все `public`/`protected` методы родителя | Только методы, которые класс явно объявил сам |

Не случайно в стандартной библиотеке Java класс `java.util.Stack` — устаревший пример наследования (`Stack extends Vector`) с той самой проблемой, а современный аналог `ArrayDeque` спроектирован через композицию.

### 3.3 Запрет наследования: final и sealed
...
### 3.4 Инкапсуляция (Encapsulation)
...
Перегрузка — это несколько методов с ОДНИМ именем, но разными списками параметров. В отличие от переопределения, здесь нет никакой связи с наследованием: компилятор просто выбирает нужную версию метода по типам и количеству переданных аргументов.
...
| `extends` | Наследование класса (is-a) |
| Композиция / агрегация / зависимость | Отношения has-a между классами (поле-владение, поле-ссылка, параметр метода) |
...
Абстрактный класс `Shape` задаёт общий контракт (метод `area()`), а каждая конкретная фигура реализует его по-своему — классический пример, где полиморфизм проявляется не в теории, а в одном вызове метода на разных объектах.
